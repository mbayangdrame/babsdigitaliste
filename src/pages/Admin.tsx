...
import { Edit, Trash2, Upload, Eye, Calendar, Folder, Tag, Plus, X, Save, Image as ImageIcon, Users, TrendingUp } from 'lucide-react';
...
const Admin: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchWithCORS(`${API_BASE_URL}/auth/login`, {
        method: 'POST',
  ...
  const fetchImages = async () => {
    try {
      const response = await fetchWithCORS(`${API_BASE_URL}/images/admin`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetchWithCORS(`${API_BASE_URL}/images/categories`);
      const data = await response.json();
      if (data.success) {
  ...
  const handleDeleteImage = async (imageId: number) => {
    ...
    try {
      const response = await fetchWithCORS(`${API_BASE_URL}/images/${imageId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
